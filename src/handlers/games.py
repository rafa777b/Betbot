...
async def play_game_callback(callback: CallbackQuery):
    ...
    if game_type == "tiger":
        ...
        # Simular subida até o target ou até o crash
    ...
    # --- LÓGICA DE RESULTADO ---
    # Se for aviator, o 'win' já foi calculado acima. Para os outros, usa a taxa padrão.
    ...
    if win:
        if game_type == "aviator":
            target = float(data.split("_")[-1])
            amount = 10 * target # Ganho proporcional ao risco
        else:
...
async def cmd_games(message: Message):
    ...
    if "aviator" in cmd or "fly" in cmd:
        # Simular clique no menu do aviator
        class Dummy:
        ...
        msg = await message.answer_photo(photo=settings.IMG_MAIN, caption="🎰 Iniciando jogo...")
        class Dummy:
